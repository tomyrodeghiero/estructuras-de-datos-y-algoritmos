...
import lista.ListaSimplementeEncadenada;
...
/**
 * La clase {@code ListaDeCompras} implementa una aplicación simple para
 * administrar una lista de compras.
 * Esta clase no utiliza parámetros, la aplicación va leyendo las entradas del
 * usuario hasta que se utilice el comando para salir.
 * 
 * @version 1.0
 */
public class ListaDeCompras {
	...
	/**
	 * El enumerado {@code Opcion} representa las distintas opciones o comandos
	 * soportados por la aplicación.
	 * Este enumerado ofrece el método estático {@code simboloOpcion()} para
	 * determinar cual es el símbolo que se debe anteponer a cada comando.
	 * Cada valor en el enumerado implementa los siguientes métodos:
	 * <ul>
	 * <li>opcion() : Retorna el comando asociado a la opción.</li>
	 * <li>descripcion() : Retorna una descripción de la opción junto a una breve
	 * descripción de su comportamiento.</li>
	 * </ul>
	 */
	public static enum Opcion {
		AGREGAR {
			...
			public String descripcion() {
				return opcion() + " <producto> <cantidad>\n" +
	...
	private static ListaSimplementeEncadenada<Articulo> listaDeCompras = new ListaSimplementeEncadenada<>();

	/**
	 * Un simple manejador de lista de compras, este programa no toma parámetros de
	 * entrada, es una aplicación interactiva.
	 * Los comandos soportados por esta aplicación están definidos en el enumerado
	 * {@code Opcion}.
	 * 
	...
	private static boolean responderAEntrada(String entrada) {
		...
		if (comando.compareToIgnoreCase(AGREGAR.opcion()) == 0) {
			if (args.length < 2) {
				System.err.println("El commando " + AGREGAR.opcion()
						+ " espera un argumento especificando el producto a agregar.");
				return false;
			}
			// String producto = args[1];
			String[] detalle = args[1].split(" ", 2);
			String producto = detalle[0];
			int cantidad = Integer.parseInt(detalle[1]);
			// TODO: agregar el producto a la lista de productos.
			Articulo nuevoArticulo = new Articulo(producto, cantidad);
			listaDeCompras.agregar(nuevoArticulo);
			return false;
		} else if (comando.compareToIgnoreCase(LISTAR.opcion()) == 0) {
			// TODO: imprimir todos los productos en la lista de productos.
			if (listaDeCompras.longitud() == 0) {
				System.out.println("No hay artículos en la Lista de Compras.");
			} else {
			...
			System.err.println("Comando " + comando + " no reconocido.\nUtilice " + AYUDA.opcion()
					+ " para obtener ayuda de como utilizar la aplicación.");
			return false;
		}
		return false;
	}
